...
function VerifyCode(props, {
	...
	const {
		...
		keyss,
		otpCode,
		setOtpCode, sendSubmit, resetComp } = otpMainVerify({ initDisabled, initDate, initOtpCode });
				...
				<SnappAuthLayout title="تایید شماره موبایل">
					...
					<div className="activationCode__form">
						<Container className="text-center">
									...
									<OtpInput
										...
										onChange={(otp) =>{
											...
											if(otp.length === props.otp.otp_length){
												setTimeout(() => {
													let inputFocus = document.querySelector('.activationCode__form .btn-success');
												inputFocus.click();
												inputFocus.focus();
												}, 1000);
							...
							<p className="midText mt-5">
								زمان باقیمانده ارسال پیامک
							</p>
